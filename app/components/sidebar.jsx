...
import Datas from '../(pages)/htmls/html.json';
import Link from 'next/link';

const Sidebar = ({ isOpen, onToggle }) => {
  return (
    <div
      className={`flex flex-col ${isOpen ? 'w-64' : 'w-20'} h-screen bg-gray-50 dark:bg-gray-800 transition-width duration-300 fixed top-0 left-0 z-40 shadow-lg`}
    >
      <button
        onClick={onToggle}
      ...
      <div className="flex flex-col space-y-4 px-3 py-4 overflow-y-auto">
        <ul className="space-y-2">
          ...
          {Datas.map((item) => (
            <li key={item.id}>
              <Link href={`/htmls/${item.id}`} className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                <svg
                  className="w-6 h-6 text-gray-500 dark:text-gray-400"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
